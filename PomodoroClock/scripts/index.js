...
const helper = {
    hasClass: (elem, className) => new RegExp(' ' + className + ' ').test(' ' + elem.className + ' '),
...
const POMORODO_MODULE = (() => {
    ...
    const setBreakLength = () => {
        const ele = helper.getElement('#break-length');
        ele.innerText = state.brkLength;
    }

    const setSessionLength = () => {
        const ele = helper.getElement('#session-length');
